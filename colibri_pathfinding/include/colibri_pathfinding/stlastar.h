	...
	unsigned int SearchStep()
	{
		...
		else // not goal
		{
			...
			for( typename vector< Node * >::iterator successor = m_Successors.begin(); successor != m_Successors.end(); successor ++ )
			{
				...
				(*successor)->parent = n;
				(*successor)->g = newg;
				float tmp_suc2goal = (*successor)->m_UserState.GoalDistanceEstimate( m_Goal->m_UserState );
				(*successor)->h = tmp_suc2goal;
				float tmp_sucf = (*successor)->g + (*successor)->h;
				(*successor)->f = tmp_sucf;

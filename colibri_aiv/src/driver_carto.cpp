...
//			if(colibri_serial::request_velocity_finished == true){
				...
				if(recv_data[CMD_CONTROL] == CMD_START){
					...
					*/
					...
					if(abs((yaw-last_yaw)*RAD_TO_DEGREE_COEFF) < DEADZONE_YAW_DEGREE)
					{
						yaw = last_yaw;
					}					
					

					ROS_INFO("virtual x/y/yaw: %0.3lfm %0.3lfm %0.2lfdeg", x, y, yaw*RAD_TO_DEGREE_COEFF);
					
		 			delta_carto_x = x - last_x;
		 			delta_carto_y = y - last_y;
		 			delta_carto_yaw = yaw - last_yaw;

					aiv_vth = delta_carto_yaw / time_period;
					...
					last_y = y;
					last_yaw = yaw;					
					ROS_INFO("virtual vx/vth: %0.3lfm/s  %0.2lfrad/s",aiv_vx,aiv_vth);

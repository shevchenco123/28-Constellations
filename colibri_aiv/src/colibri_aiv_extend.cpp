...
//			if(colibri_serial::request_velocity_finished == true){
				...
				if(recv_data[CMD_CONTROL] == CMD_START){
					...
					double dx = cos(aiv_dth) * aiv_dx;
					double dy = -sin(aiv_dth) * aiv_dx;
					
					aiv_angle += aiv_dth;

                                        if(recv_data[DATA_BYTE + 6] == 0x00)
					{
                                                aiv_pose_trans_x = (0.01) * (float)((recv_data[DATA_BYTE + 7] << 8)|recv_data[DATA_BYTE + 8]);
                                        }else if(recv_data[DATA_BYTE + 6] == 0xff)
					{
                                                aiv_pose_trans_x =(-0.01) * (float)((recv_data[DATA_BYTE + 7] << 8)|recv_data[DATA_BYTE + 8]);
                                        }else
					{
						cout<<"The [DATA_BYTE + 6] of the request_velocity cmd is illegal !"<<endl;
                                                return;
                                        }

                                        if(recv_data[DATA_BYTE + 9] == 0x00)
					{
                                                aiv_pose_trans_y = (0.01) * (float)((recv_data[DATA_BYTE + 10] << 8)|recv_data[DATA_BYTE + 11]);
                                        }else if(recv_data[DATA_BYTE + 9] == 0xff)
					{
                                                aiv_pose_trans_y = (-0.01) * (float)((recv_data[DATA_BYTE + 10] << 8)|recv_data[DATA_BYTE + 11]);
                                        }else
					{
                                                cout<<"The [DATA_BYTE + 9] of the request_velocity cmd is illegal !"<<endl;
                                                return;
                                        }

                                        if(recv_data[DATA_BYTE + 12] == 0x00)
					{
                                                aiv_pose_rotate_yaw = (0.01) * (float)((recv_data[DATA_BYTE + 13] << 8)|recv_data[DATA_BYTE + 14]);                                        }
					else if(recv_data[DATA_BYTE + 12] == 0xff)
					{
                                                aiv_pose_rotate_yaw = (-0.01) * (float)((recv_data[DATA_BYTE + 13] << 8)|recv_data[DATA_BYTE + 14]);
                                        }else
					{
                                                cout<<"The [DATA_BYTE + 12] of the request_velocity cmd is illegal !"<<endl;
                                                return;
                                        }


					
					cout<<"aiv_pose_trans_x: "<<aiv_pose_trans_x<<" m"<<endl;
					cout<<"aiv_pose_trans_y: "<<aiv_pose_trans_y<<" m"<<endl;
					cout<<"aiv_pose_rotate_yaw: "<<aiv_pose_rotate_yaw*57.3<<" degree"<<endl;
					geometry_msgs::Quaternion odom_quat = tf::createQuaternionMsgFromYaw(aiv_angle);
					...
					odom_trans.transform.translation.x = aiv_pose_trans_x;
					odom_trans.transform.translation.y = aiv_pose_trans_y;
					...
					//set the position
					odom.pose.pose.position.x = aiv_pose_trans_x;
					odom.pose.pose.position.y = aiv_pose_trans_y;
					...
					odom.twist.twist.angular.z = aiv_vth;
...
bool colibri_serial::init_sub_and_pub()
{
	...
	twist_sub= global_nh.subscribe<geometry_msgs::Twist>("cmd_vel", 1, boost::bind(&colibri_serial::twist_callback, this, _1));

...
void LMS1xx::startMeas()
{
  ...
  if (buf[0] != 0x02)
    logWarn("invalid packet recieved in startMeas!");
  buf[len] = 0;
  logDebug("RX: %s", buf);
}
...
void LMS1xx::stopMeas()
{
  ...
  int len = read(socket_fd_, buf, 100);
  if (buf[0] != 0x02)
    logWarn("invalid packet recieved in stopMeas!");
  buf[len] = 0;
  logDebug("RX: %s", buf);
}
...
status_t LMS1xx::queryStatus()
{
  ...
  int len = read(socket_fd_, buf, 100);
  if (buf[0] != 0x02)
    logWarn("invalid packet recieved in queryStatus!");
  buf[len] = 0;
  logDebug("RX: %s", buf);
...
void LMS1xx::login()
{
  char buf[100];
  int result;
  sprintf(buf, "%c%s%c", 0x02, "sMN SetAccessMode 02 B21ACE20", 0x03);
  ...
  int len = read(socket_fd_, buf, 100);
  if (buf[0] != 0x02)
    logWarn("invalid packet recieved in login!");
  buf[len] = 0;
  logDebug("RX: %s", buf);
}
...
scanCfg LMS1xx::getScanCfg() const
{
  ...
  int len = read(socket_fd_, buf, 100);
  if (buf[0] != 0x02)
    logWarn("invalid packet recieved in getScanCfg!");
  buf[len] = 0;
  logDebug("RX: %s", buf);
...
void LMS1xx::scanContinous(int start)
{
  ...
  if (buf[0] != 0x02)
    logError("invalid packet recieved in scanContinous");
...
void LMS1xx::saveConfig()
{
  ...
  if (buf[0] != 0x02)
    logWarn("invalid packet recieved in saveConfig!");
  buf[len] = 0;
  logDebug("RX: %s", buf);
}
...
void LMS1xx::startDevice()
{
  ...
  if (buf[0] != 0x02)
    logWarn("invalid packet recieved in startDevice!");
  buf[len] = 0;
  logDebug("RX: %s", buf);
}

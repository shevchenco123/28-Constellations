...
int main(int argc, char** argv)
{
  ...
  if(!ac.waitForServer(ros::Duration(20)))
  {
    ROS_INFO("Can't connected to move base server");
    return 1;
  }
  ROS_INFO("Connected to move base server");
  ROS_INFO("Starting two goal test");

  //Initialize a counter to track waypoints
  int count = 0;
  //Cycle through the four waypoints
  while( (count < 2) && (ros::ok()) )
  {
    ...
    bool finished_within_time = ac.waitForResult(ros::Duration(90));

    //If we dont get there in time, abort the goal
    if(!finished_within_time)
    {
        ac.cancelGoal();
        ROS_INFO("Timed out achieving goal %d", count);
    }
    else
    {
        //We made it!
        ...
        {
            ROS_INFO("Goal Index %d :succeeded!", count);
        }
        else
        {

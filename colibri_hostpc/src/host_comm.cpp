...
int main(int argc, char*argv[])
{
    ...
    if (argc > 1) {
        if (strcmp(argv[1], "tcp") == 0) {
            ...
            printf("Usage:\n  %s [tcp|rtu] - Modbus server for communication testing\n\n", argv[0]);
            return -1;
        }
    } else {
        /* By default */
    ...
    if (use_backend == RTU) {
		...
		ctx = modbus_new_tcp("192.168.100.100", 1502);
        query = (uint8_t *)malloc(MODBUS_TCP_MAX_ADU_LENGTH);
    } else {
		printf("wrong modbus communication mechanism...");
	}
	...
	ros::init(argc, argv, "hostpc_comm_node");
	ros::NodeHandle n;
